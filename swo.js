(function () {
    ...
    /**
     * Filmix Nexus (Series Pro Fix) v2.1.1
     * - BASE_DOMAIN changed to IP from user's link to avoid resolution issues
     * - Parsing priority: series first
     * - Menu only if no series found
     * - No voice name filtering
     * - Added logging for debugging (remove in production)
     */
        ...
        function FilmixComponent(object) {
            ...
            this.loadContent = function(custom_url) {
                ...
                network.native(PROXIES[currentProxyIdx] + url, function (res) {
                    safeLoading.hide();
                    console.log('Response from URL:', url, res); // Для дебага
                    self.parseData(res);
                }, function () {
                    safeLoading.hide();
            ...
            this.parseData = function(res) {
                ...
                console.log('Parsed voice_links:', voice_links); // Для дебага
                console.log('Parsed raw_data:', raw_data); // Для дебага
