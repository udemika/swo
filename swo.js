(function () {
    ...
    /**
     * Filmix Nexus (Series Pro Fix) v2.0.9 – FINAL FIX
     * - Если в ответе есть серии → показываем их сразу (даже если есть кнопки озвучек)
     * - Меню озвучек только когда серий нет совсем
     * - Название выбранной озвучки добавляется в заголовок серии
     * - Максимально упрощена и стабилизирована логика
     */
    function startPlugin() {
        ...
        function FilmixComponent(object) {
            ...
            this.parseData = function(res) {
                ...
                // Главное правило: если серии есть — показываем их сразу
                if (raw_data.length > 0) {
                    this.renderList();
                }
                // Только если серий НЕТ, но есть озвучки — показываем меню
            ...
            this.renderList = function() {
                ...
                raw_data.forEach(data => {
                    var title = data.title?.trim() || (`Серия ${data.e || '?'}`);

                    if (filters.voice !== 'Любой' && filters.voice !== 'Стандарт') {
